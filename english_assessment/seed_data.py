...
def seed_assessments():
    ...
    with app.app_context():
        ...
        # ---------------------------------------------------------------
        # 24. 2do SECUNDARIA - A2: Adverbs of Frequency
        # ---------------------------------------------------------------
        a = Assessment(
        ...
        freq_options = [('always', False), ('usually', False), ('often', False),
                        ('sometimes', False), ('never', False)]

        def mc_freq(text, instruction, order, correct_adverb):
            opts = [(adv, adv == correct_adverb) for adv, _ in freq_options]
        ...
        db.session.commit()
        all_codes.append(('FREQ2SEC', '2nd Secondary', 'Elementary (A2)', a.title))
        print(f'  Created: {a.title}')
